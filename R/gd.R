...
l2shm.sgd.emp <- function(U, tmin, groups = 1L, par = rand.par.list.emp(U, groups, tmin), batch = 1L, maxiter = 1L, tol = 10^-5, terms = 50L){
  invisible(.Call("_stoch_grad_desc_emp", PACKAGE="l2shm", U, tmin, par$mu, par$t, par$alpha, batch, maxiter, tol, terms))
}

l2shm.sgd.proj <- function(par0, groups = 1L, par = rand.par.list.proj(par0, groups), batch = 1L, maxiter = 1L, tol = 10^-5, terms = 50L){
  invisible(.Call("_stoch_grad_desc_proj", PACKAGE="l2shm", par0$mu, par0$t, par0$alpha, par$mu, par$t, par$alpha, batch, maxiter, tol, terms))
}
